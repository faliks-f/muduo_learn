...
std::string faliks::Timestamp::toString() const {
    char buf[32] = {0};
...
std::string faliks::Timestamp::toFormattedString(bool showMicroseconds) const {
    char buf[64] = {0};
    auto seconds = static_cast<time_t>(m_microSecondsSinceEpoch / kMicroSecondsPerSecond);
    struct tm tm_time{};
...
faliks::Timestamp faliks::Timestamp::now() {
    struct timeval tv{};
